...
class imp_res : public Restaurant
{
	...
	public:	
		...
		void addLeft(customer* &current, customer* &cus) {
			...
			if(current->prev != nullptr) current->prev->next = cus;
			current->prev = cus;
		}
		...
		void addnewcus () {
			while(queue_counter > 0 && cus_counter < MAXSIZE) {
				...
				else if(newcus == queue_head) {
					queue_head = queue_head->next;
					queue_head->prev = NULL;
					newcus->next = NULL;
				}
				else if(newcus == queue_tail) {
					queue_tail = queue_tail->prev;
					queue_tail->next = NULL;
					newcus->prev = NULL;
				}
				else {
					newcus->prev->next = newcus->next;
					newcus->next->prev = newcus->prev;
				}
				delete newcus;
				queue_counter--;
				// print_queue();
		...
		void UNLIMITED_VOID()
		{
			...
			for(int i = 0; i < cus_counter ; i++) {
				...
				for(int j = 1; j < cus_counter -1 && element_counter < cus_counter; j++) {
